...
export function SacredCard({ children, className = '', glow = false, overflowVisible = false }: SacredCardProps) {
  return (
    <div className={`sacred-card rounded-[2.5rem] p-8 md:p-12 relative group ${overflowVisible ? '!overflow-visible' : 'overflow-hidden'} ${glow ? 'glow-amber' : ''} ${className}`}>
